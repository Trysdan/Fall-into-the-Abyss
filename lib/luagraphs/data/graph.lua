...
function graph.create(V, directed)
    ...
    g.vertexList = require('lib.luagraphs.data.list').create()
    g.adjList = {}
    for v = 0,V-1 do
        g.vertexList:add(v)
        g.adjList[v] = require('lib.luagraphs.data.list').create()
    end
    g.directed = directed
...
function graph:addVertexIfNotExists(v)
    if self.vertexList:contains(v) then
        ...
        self.adjList[v] = require('lib.luagraphs.data.list').create()
        return true
    end
end
...
function graph:edges()
    local list = require('lib.luagraphs.data.list').create()
