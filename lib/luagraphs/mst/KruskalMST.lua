...
function KruskalMST.create()
    ...
    s.marked = {}
    s.path = require('lib.luagraphs.data.list').create()
    return s
end

function KruskalMST:run(G)
    self.marked = {}
    self.path = require('lib.luagraphs.data.list').create()
    local pq = require('lib.luagraphs.data.MinPQ').create(function(e1, e2)
    ...
    local uf = require('lib.luagraphs.data.UnionFind').createFromVertexList(G:vertices())
